...
//TODO fix collision
//TODO write doSomething for each class
//TODO write return cases for no more levels left
	...
	else if (result == Level::load_success)
	{
		...
		for (int x = 0; x < 16; x++) {
			for (int y = 0; y < 16; y++) {
				...
				case Level::exit:
					m_objList.push_back(new Exit(SPRITE_WIDTH * x, SPRITE_HEIGHT * y, this));
					break;
				}
			}
		}
...
int StudentWorld::move()
{
	...
	//cout << "2" << endl;
	for (vector<Actor*>::iterator p = m_objList.begin(); p != m_objList.end(); p++)
		(*p)->doSomething();
	//cout << "3" << endl;
	if (m_levelFinish) {
		m_levelFinish = false;
